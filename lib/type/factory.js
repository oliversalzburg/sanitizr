 ...
 * @param {mongoose.Schema} schema The schema that was used to construct the type.
 ...
 */
function assemble( typeName, collectionName, typeDescription, populator ) {
	// Create a schema
 ...
 */
function extend( typeName, baseName, typeDescription, populator ) {
 ...
 * @returns {Type} The type metadata container for the derived type.
 */
function extendWithCollection( typeName, baseName, collectionName, typeDescription, populator ) {
	var baseTypeInfo = TypeInfo.types[ baseName ];
	if( !baseTypeInfo ) {
		throw new Error( "Invalid base type '" + baseName + "'." );
	}
