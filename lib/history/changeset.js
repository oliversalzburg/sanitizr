...
ChangeSet.from = function ChangeSet$from( base, head ) {
	var changeSet = new ChangeSet();

	if( typeof base != typeof head ) {
		changeSet.push( {
			type : ChangeSet.TYPE_TYPE
		} );
		return changeSet;
	}
	...
	while( baseIndex < baseKeys.length && headIndex < headKeys.length ) {
		if( baseKeys[ baseIndex ] != headKeys[ headIndex ] ) {
			...
			if( 1 === next ) {
				...
				changeSet.push( {
					type : ChangeSet.TYPE_ADDITION,
					key  : headKeys[ headIndex ]
				} );
				++headIndex;
			} else {
				changeSet.push( {
					type : ChangeSet.TYPE_DELETION,
					key  : baseKeys[ baseIndex ]
				} );
				++baseIndex;
			}
	...
	if( baseIndex < baseKeys.length ) {
		for( ; baseIndex < baseKeys.length; ++baseIndex ) {// jscs:ignore disallowSpaceBeforeSemicolon
			changeSet.push( {
				type : ChangeSet.TYPE_DELETION,
				key  : baseKeys[ baseIndex ]
			} );
	...
	if( headIndex < headKeys.length ) {
		for( ; headIndex < headKeys.length; ++headIndex ) {// jscs:ignore disallowSpaceBeforeSemicolon
			changeSet.push( {
				type : ChangeSet.TYPE_ADDITION,
				key  : headKeys[ headIndex ]
			} );
